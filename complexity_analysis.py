...
def run_complexity_analysis(name, xlabel, ylabel, create_estimator_w_parameter, parameter_values, features, classes, scorer, folds=10, pause_at_end=False, save_csv=False, x_log_space=False):
    ...
    for iteration, parameter_value in enumerate(parameter_values):
        ...
        # plot current
        if iteration > 0:
            plot_complexity_analysis(name, xlabel, ylabel, parameter_values[0:iteration + 1], training_scores_means,
    ...
    plot_complexity_analysis(name, xlabel, ylabel, parameter_values, training_scores_means, training_scores_stds,
                             validation_scores_means, validation_scores_stds, x_log_space=x_log_space)
    plot.savefig('%s_%s' % (name.replace(' ', '_').lower(), xlabel.replace(' ', '_')))
    if pause_at_end:
        plot.ioff()
        plot.show()
...
def save_data(filename_w_o_extension, data):
    filename_to_try = '%s.csv' % filename_w_o_extension
    count = 0
    column_names = []
    length = 0
    for column in data:
    ...
    while True:
        try:
            ...
            if err.filename == filename_to_try:
                count += 1
                filename_to_try = '%s_%d.csv' % (filename_w_o_extension, count)
            else:
                return
    
def general_accuracy(estimator, X, y):
    predicted_classes = estimator.predict(X)
    return numpy.sum(numpy.array(predicted_classes) == numpy.array(y)) / len(y)
...
def run_learning_curve_analysis(name, estimator, training_features, training_classes, folds=5):
    ...
    #plot.ioff()
    #plot.show()
    plot.savefig('%s_learning_curve' % name.replace(' ', '_').lower())
